...
Zhao (2026) 证明了专家级提示词会诱导"深层扩展"——深层的有效内在维度（EID）增加 60-100%。然而，EID 是一个全局指标，无法揭示**哪些语义特征**被激活。本文应用稀疏自编码器（SAE）分析来分解不同提示词风格之间的激活差异。使用 Goodfire 的 Llama-3.3-70B SAE（第 50 层，65,536 个特征），我们发现：(1) "向新手解释"比"向专家解释"激活了**多 17% 的特征**（平均 132.4 vs 113.1）；(2) **369 个特征仅被新手提示激活**，而专家提示仅激活 208 个；(3) **10 个特征在 Novice vs Expert 之间实现完美分离**；(4) 通过 AutoInterp 分析（6 种条件 × 50 主题 = 300 样本），我们发现这些特征具有**语义细分结构**——分别编码「专家身份」「认真态度」「深度要求」「技术分析」等不同维度。这些发现表明提示词的效果是**可组合的**，不同元素触发不同的特征子集。
