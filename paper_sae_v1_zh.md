...
**讨论论文 v2** — 基于 [Zhao (2026)](https://zenodo.org/records/18410085) 的稀疏自编码器分析扩展

---

## 摘要

Zhao (2026) 证明了专家级提示词会诱导"深层扩展"——深层的有效内在维度（EID）增加 60-100%。然而，EID 是一个全局指标，无法揭示**哪些语义特征**被激活。本文应用稀疏自编码器（SAE）分析来分解不同提示词风格之间的激活差异。使用 Goodfire 的 Llama-3.3-70B SAE（第 50 层，65,536 个特征），我们发现：(1) "向新手解释"比"向专家解释"激活了**多 17% 的特征**（平均 132.4 vs 113.1）；(2) **369 个特征仅被新手提示激活**，而专家提示仅激活 208 个；(3) **10 个特征实现完美分离**（一种条件下 100% 激活，另一种 0%）。这些发现为提示词诱导的 EID 差异提供了机制证据，表明这反映了不同的**稀疏特征激活模式**，而非统计噪声。
...
### 3.5 特征语义分析：提示词驱动 vs 主题驱动

为了验证这些特征确实是**提示词驱动**而非**主题驱动**，我们检查了它们在所有 50 个技术主题上的激活模式：

| 特征 ID | 类型 | 激活率 | 平均激活值 | 语义推断 |
|---------|------|--------|-----------|----------|
| **Novice 独占** ||||
...
这意味着：
- 无论问的是 Raft 共识、Docker 容器还是 SQL 注入
- 只要用 Novice 提示词，那 4 个特征就**必定**激活
- 只要用 Expert 提示词，那 6 个特征就**必定**激活

**结论：这些是纯粹的提示词驱动特征，而非主题驱动特征。** 它们构成了模型切换"教学模式"和"专业模式"的**神经开关**。
...
基于 3.5 节的发现，我们提出**模式开关假说**：

> 大语言模型内部存在专门的"模式切换"特征，用于区分不同的交流情境（教学 vs 专业讨论）。这些特征在提示词解析阶段被激活，并持续影响后续的生成过程。

具体而言：
- **Feature 34942/59519**：可能是"激活更多背景知识"的信号
- **Feature 46703/21604**：可能是"假设受众具有专业知识"的信号

这与人类交流中的"受众感知"类似——我们会根据听众调整解释的详细程度。模型似乎在第 50 层（约 60% 深度）就已经形成了这种区分。
...
3. **10 个特征在两种条件之间实现完美分离**（100% vs 0%）
4. **这些特征是提示词驱动的，不是主题驱动的**——无论问什么问题，只要用特定提示词就会激活
5. **激活强度不变**——效果在于哪些特征被激活，而非激活强度

对提示工程的启示：
- **"向新手解释"可能比"作为专家解释"更强大**——因为教学迫使模型激活更多的知识
- **模型内部存在"模式开关"**——可以通过提示词精确控制
- **这些开关是可发现的**——SAE 提供了系统性识别它们的方法
...
- **特征分析：** 仓库中的 `feature_diff.json`、`feature_context.json`

---

**版本历史：**
- v1 (2026-01-31)：初始发布，激活计数和独占特征分析
- v2 (2026-02-01)：新增 3.5 节特征语义分析，提出"模式开关"假说
