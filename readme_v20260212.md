...
> **EID = exp(Shannon_Entropy(normalized_singular_values))**：对 Layer 50 整个序列的 hidden_states 做 SVD，计算有效内在维度。
>
> 运行 `python step_eid_persona.py` 生成 `eid_persona_results.json`，结果将补充到此处。

### 关键发现

1. **standard/assistant 完美一致**（SAF 113.3/113.3）— 实验可复现性验证通过
2. **novice (129.1) > expert (100.6)**，SAF 差距 28% — 复现原实验结论
3. **苏格拉底式提问 SAF 最高（142.8）**，比基线高 26%。提问引导模式需要同时激活"问题生成"和"概念关联"两套回路
4. **critic (129.6) 第三高** — 批判需要先理解再挑刺，激活的语义单元多
5. **expert (100.6) 和 guru (98.4) SAF 排名倒数** — "越专业越少激活"，与 novice 形成鲜明对比
6. **助手轴反面（villain/drunk）SAF 较少**（103/101），但不是最低 — 最低的是 child (84.6) 和 eli5 (88.8)
7. **极度简化比"变坏"更能压缩激活** — child/eli5 SAF 最少但峰值强度最高（4.82/5.48），少数 feature 被猛烈激活
8. **SAF 反映认知复杂度，分三个层级**：
   - **高 SAF（>125）**：socratic、critic、novice — 需要多角度展开
   - **中 SAF（100~120）**：standard、debugger、interviewer、poet、teacher、conspiracy — 正常回答
   - **低 SAF（<100）**：expert、guru、eli5、child — 要么太专、要么太简
